...
const GOOGLE_SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbyoEq00QQ9frYqccma_alfnYSxVqdhnPteoX6zJc96efyk2bmjzVH8EE1_Jau0S6yY14g/exec";
...
const Button = ({
  ...
  const baseStyle =
    "px-4 py-3 rounded-xl font-bold transition-all transform active:scale-95 flex items-center justify-center gap-2 shadow-sm";
  const variants = {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubClients = onSnapshot(
      query(collection(db, "artifacts", appId, "public", "data", "clients")),
      (snapshot) => {
    ...
    const unsubProducts = onSnapshot(
      query(collection(db, "artifacts", appId, "public", "data", "products")),
      (snapshot) => {
        const data = snapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
    ...
    const unsubEntries = onSnapshot(
      query(collection(db, "artifacts", appId, "public", "data", "entries")),
      (snapshot) => {
        const data = snapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
  ...
  const seedDefaultUsers = async () => {
    ...
    await addDoc(
      collection(db, "artifacts", appId, "public", "data", "users"),
      { username: "staff", password: "1234", role: "staff", name: "Gate Staff" }
    );
  };
  ...
  const LoginScreen = () => {
    ...
    const handleLogin = (e) => {
      e.preventDefault();
      // Master Override (Hidden from view, allows you to login if you get locked out)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-700 via-red-600 to-yellow-500 flex items-center justify-center p-6 font-sans">
        <div className="max-w-sm w-full bg-white rounded-2xl shadow-2xl overflow-hidden relative">
          ...
          <div className="pt-10 pb-6 px-8 text-center">
            <div className="relative w-full h-24 flex items-center justify-center mb-6 group">
              {appLogo ? (
                <img
                  src={appLogo}
                  alt="Tirth Industries"
              ...
              <label className="absolute inset-0 bg-white/90 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity rounded-lg border-2 border-dashed border-gray-300">
                <Upload size={24} className="text-red-500 mb-1" />
          ...
          <form onSubmit={handleLogin} className="px-8 pb-8 space-y-5">
            {error && (
              <div className="bg-red-50 border border-red-100 text-red-600 text-xs font-medium p-3 rounded-lg flex items-center gap-2">
                <XCircle size={14} /> {error}
              </div>
            )}
            <div className="space-y-4">
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400 group-focus-within:text-red-600">
                  <User size={20} />
                </div>
                <input
                  type="text"
            ...
            <Button
              type="submit"
              variant="primary"
              className="w-full py-3.5 shadow-red-200"
            >
            ...
            >
              {editId === item.id ? (
                <div className="flex gap-2 flex-1 mr-2">
                  ...
                  />
                  <button
                    onClick={() => saveEdit(collectionName)}
              ...
              {editId !== item.id && (
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button
                    onClick={() =>
                      handleDelete(collectionName, item.id, item.name)
                    }
      ...
      <div className="space-y-6">
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
          <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2 text-sm uppercase tracking-wide">
            <UserPlus size={18} className="text-red-600" /> Create New User
          </h3>
          <form
          ...
          >
            ...
            <div className="md:col-span-2">
              <Button type="submit" variant="brand" className="w-full py-2">
                Create User Account
              </Button>
            </div>
          </form>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-4 bg-gray-50 border-b">
            <h3 className="font-bold text-gray-800 text-sm">Active Accounts</h3>
          </div>
              ...
              <div key={u.id} className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    @{u.username} • <span className="uppercase">{u.role}</span>
                  </p>
                </div>
                {u.username !== "admin" && (
                  <button
        ...
        <div className="flex gap-2 bg-gray-100 p-1 rounded-xl mb-4 overflow-x-auto">
          {["clients", "products", "users"].map((tab) => (
            <button
              ...
              className={`flex-1 py-2 px-4 capitalize text-sm font-bold rounded-lg transition-all ${
                activeTab === tab
  ...
  const EntryForm = () => {
    const [formData, setFormData] = useState({
      ...
      totalWeight: 0,
      transportCharges: "",
      transportMode: "Client Transport",
      remarks: "",
      qcStatus: "Pending",
      plateImage: null,
    });
    ...
    const handlePlateImage = (e) => {
      ...
      if (file) {
        const reader = new FileReader();
        reader.onloadend = () =>
          setFormData((prev) => ({ ...prev, plateImage: reader.result }));
        reader.readAsDataURL(file);
      }
    };
    ...
    const handleSaveEntry = async (e) => {
      e.preventDefault();
      if (!firebaseUser) return;
      if (!formData.clientName || !formData.productName || !formData.vehicleNo)
    ...
    return (
      <form
        ...
        className="max-w-3xl mx-auto space-y-5 pb-24"
      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <InputGroup label="Transport Mode">
              <select
                ...
                onChange={handleChange}
                className="w-full p-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-red-500"
              >
                <option>Client's Transport</option>
        ...
        <Card className="p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {!isAddingClient ? (
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="w-full p-3 border border-red-300 rounded-xl"
                  />
                  <button
                    type="button"
                    onClick={addNewClient}
                    className="px-4 bg-red-600 text-white rounded-xl"
                  >
                    Add
                  </button>
                  <button
            ...
            <div className="grid grid-cols-2 gap-3 md:col-span-2">
              <InputGroup label="No. of Bags">
                <input
                  ...
                  value={formData.bags}
                  onChange={handleChange}
                  className="w-full p-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-red-500 font-bold"
                />
              </InputGroup>
            ...
            <div className="md:col-span-2 bg-yellow-50 border border-yellow-200 p-4 rounded-xl flex justify-between items-center">
              <span className="text-yellow-800 text-xs font-bold uppercase">
                Total Quantity
              </span>
              <div className="text-3xl font-extrabold text-yellow-900">
                {formData.totalWeight.toLocaleString()}{" "}
            ...
            <div>
              ...
              <div className="border-2 border-gray-200 border-dashed rounded-xl bg-gray-50 touch-none relative">
                ...
                <button
                  ...
                  className="absolute bottom-2 right-2 text-xs bg-white border px-2 py-1 rounded shadow-sm text-gray-500"
                >
                  Clear
                </button>
              </div>
                ...
                <div className="flex gap-2">
                  {["Pending", "Approved", "Rejected"].map((status) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setFormData((prev) => ({ ...prev, qcStatus: status }))
                      }
        ...
        <Button
          type="submit"
          variant="primary"
          className="w-full py-4 text-lg shadow-xl shadow-red-200"
        >
  ...
  const Dashboard = () => {
    ...
    const updateStatus = async (id, newStatus) => {
      await updateDoc(
        doc(db, "artifacts", appId, "public", "data", "entries", id),
        { qcStatus: newStatus }
      );
    };
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-500 flex gap-3">
                    <span className="flex items-center gap-1">
                      <Package size={12} /> {entry.productName}
                    </span>
                    <span className="flex items-center gap-1 font-mono text-gray-700">
                      <Truck size={12} /> {entry.vehicleNo}
                    </span>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 p-4 border-t border-gray-100 text-sm">
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <span className="font-medium">{entry.lotNo || "-"}</span>
                    </div>
                    <div>
                      <span className="text-gray-400 block text-xs">
                        Transport
                      </span>{" "}
                      <span className="font-medium">{entry.transportMode}</span>
                    </div>
                    <div>
                      <span className="text-gray-400 block text-xs">
                        Driver
                  ...
                  <div className="flex justify-end gap-2 pt-2 border-t border-gray-200">
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-2 bg-green-100 text-green-700 rounded-lg font-bold text-xs"
                    >
                      Approve
                    </button>
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-2 bg-red-100 text-red-700 rounded-lg font-bold text-xs"
                    >
                      Reject
                    </button>
                  </div>
  ...
  const TodayTableWrapper = () => {
    return <TodayTable />;
  };

  if (loading)
    return (
      <div className="h-screen flex items-center justify-center text-red-600 font-bold bg-white">
        ...
        <div className="max-w-4xl mx-auto flex justify-between items-center h-14">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-red-100 opacity-90 font-medium tracking-wide uppercase">
                Spices & Seasoning
              </p>
            </div>
          </div>
          <button
            onClick={() => setIsAppLoggedIn(false)}
            className="bg-red-800/50 p-2 rounded-lg hover:bg-red-800 text-white/90 backdrop-blur-sm transition-colors"
          >
            <LogOut size={18} />
          </button>
        </div>
